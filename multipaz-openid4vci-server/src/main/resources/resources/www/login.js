(function() {
    ...
    const isLoggedIn = function() {
        for (let cookie of document.cookie.split(/\s*;\s*/)) {
            if (cookie.startsWith("issuance_auth=")) {
                return true;
            }
        }
        return false;
    ...
    if (login) {
        ...
        logOutButton.addEventListener("click", function() {
            document.cookie =
                "issuance_auth=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=" + basePath + ";"
            loginRefresh();
        });
        ...
        const logIn = async function() {
            let response = await(await fetch(basePath + "admin_auth", {
                method: 'POST',
                headers: {
